# OmniQL FFI / C API Reference — v0.7.0
...
#### `OmniQL_RegisterMySQLDriver(int handle, const char* dsn) → char*`
...
#### `OmniQL_RegisterSQLServerDriver(int handle, const char* dsn) → char*` *(new in v0.7.0)*
Opens a SQL Server / Azure SQL connection and registers the driver.

DSN format: `"sqlserver://user:password@host:port?database=dbname"`

Returns `{"driver":"sqlserver"}` on success.
...
#### `OmniQL_RegisterRedisDriver(int handle, const char* url) → char*` *(new in v0.7.0)*
Connects to Redis using the given URL and registers the KV driver.

URL format: `"redis://:password@host:port/db"` (standard Redis URL)
...
```c
char* r = OmniQL_Route(handle, "orders", "sqlserver");
OmniQL_Free(r);
char* r2 = OmniQL_Route(handle, "sessions", "redis");
OmniQL_Free(r2);
```
...
```json
{
  ...
  "meta": { "total": 1, "returned": 1, "driver": "sqlserver", "target": "orders" },
  "error": null
}
```
...
```json
{
  ...
  "error": { "code": "EXECUTION_ERROR", "message": "mssql: Invalid object name 'orders'" }
}
```
...
OmniQL_RegisterSQLServerDriver(engine1,
    "sqlserver://sa:Pass@localhost:1433?database=app");
OmniQL_RegisterRedisDriver(engine2, "redis://localhost:6379/0");
...
| `OmniQL_RegisterSQLServerDriver` | **v0.7.0** |
| `OmniQL_RegisterRedisDriver` | **v0.7.0** |
